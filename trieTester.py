...
def lookupTest(t, lstToLookup):
    ...
    for (w, k) in lstToLookup:
        j = t.lookupWord(w)
        if (j != k):
            print('\t Lookup for word %s failed. Expected result: %d,'
                  'obtained from your trie: %d \n' % (w, k, j))
            retValue = False
    return retValue
    ...
    try:
        ...
        for line in file:
            ...
            if (lst[0] == 'W'):
                ...
                print('Error in test specification line number %d -- '
                      'Unknown command spec %s' % (lNum, lst[0]))
                sys.exit(1)
        return result
    except IOError:
